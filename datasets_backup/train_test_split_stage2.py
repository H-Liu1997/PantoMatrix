...
motion_folder = "./Forbes/cache_latent_v1/"
...
train_names = list(set(raw_video_names) - set(test_wild_names))
...
for test_wild_name in tqdm(test_wild_names):
    ...
    for sample in sample_with_name:
        ...
        motion_self_path = os.path.join(motion_folder, f"{file_name}_0.npz")
        motion_other_path = os.path.join(motion_folder, f"{file_name}_1.npz")
        if not os.path.exists(motion_self_path):
            print(motion_self_path, "not exists")
            continue
        if not os.path.exists(motion_other_path):
            print(motion_other_path, "not exists")
            continue 
        ...
        for i in range(0, total_len - motion_length, stride):
            clips.append({
                ...
                "motion_self_path": motion_self_path,
...
out_json = os.path.join(output_dir, f"dyanic_fliter_s{stride}_l{motion_length}_v1.json")
