...
def train_val_fn(cfg, batch, model, device, mode="train", **kwargs):
    ...
    face_codebook = motion_vq.vq_model_face.quantizer.embedding
    motion_pred = model(audio, speaker_id, masked_motion=masked_motion, mask=mask, codebook=face_codebook, use_audio=True)
    loss_dict = {
