...
class CustomDataset(Dataset):
    def __init__(self, args, loader_type, augmentation=None, kwargs=None, build_cache=True):
        ...
        self.smplx = smplx.create(
            self.args.data_path_1+"/smplx_v1.1/models/",
            model_type='smplx',
            gender='NEUTRAL_2020',
        ...
        logger.info(f"customdataset init done! LDMB cache with:{self.n_samples}samples")
    ...
    def calculate_mean_velocity(self, save_path):
        ...
        for tar in tqdm(os.listdir(dir_p)):
            ...
            # first steps is forward diff (t+1 - t) / dt
                ...
                vel_seq = torch.cat([init_vel, middle_vel, final_vel], dim=1).permute(1, 0).reshape(n, 54, 3)
                #print(vel_seq.shape)
    ...
    def cache_generation(self, out_lmdb_dir, disable_filtering, clean_first_seconds,  clean_final_seconds, is_test=False):
        ...
        if len(self.args.training_speakers) == 1:
            dst_lmdb_env = lmdb.open(out_lmdb_dir, map_size= int(1024 ** 3 * 15))# 50G
        else:
            dst_lmdb_env = lmdb.open(out_lmdb_dir, map_size= int(1024 ** 3 * 15))# 200G
        ...
        for index, file_name in self.selected_file.iterrows():
            ...
            if "smplx" in self.args.pose_rep:
                ...
                # print(f"pose_each shape{pose_each_file.shape}")
                # j 9 --> j 8 --> [:, 3:]
                # pose_each_file = pose_each_file[:, 3:]
                # print(f"pose_each 1 shape{pose_each_file.shape}")
                ...
                # print(f"pose_each_file 2 shape{pose_each_file.shape},joint mask shape{self.joint_mask.shape}")

                pose_each_file = pose_each_file * self.joint_mask
                # print(f"pose_each 3 shape{pose_each_file.shape}")
                pose_each_file = pose_each_file[:, self.joint_mask.astype(bool)]

                # print("pose shape",pose_each_file.shape)
                # 30 fps
                dt = 1 / 30
                print("rot-pos shape", pose_each_file.shape)
                # rot-vel
                angular_velocity = np.gradient(pose_each_file, axis=0) / dt
                print("rot-vel shape", angular_velocity.shape)
                #
                # # angular_velocity
                # dt = 1/30
                # rotation_poses = torch.from_numpy(pose_each_file[:, :165]).reshape(-1, 55, 3)
                # angular_velocity = (rotation_poses[1:] - rotation_poses[:-1]) / dt
                # angular_velocity = torch.cat([angular_velocity[:1], angular_velocity], dim=0).reshape(-1, 165).cpu().numpy()
                # print("rot-vel shape", angular_velocity.shape)
                ...
                for i in range(s):
                    with torch.no_grad():
                        joints = self.smplx(
                            ...
                            reye_pose=poses[i*max_length:(i+1)*max_length, 72:75],
                        # lower
                        # )['joints'][:, (1,2,4,5,7,8,10,11), :].reshape(-1, 8, 3).cpu()
                        # full
                        )['joints'][:, :54, :].reshape(-1, 54, 3).cpu()
                ...
                if r != 0:
                    ...
                    positions.append(joints)
                joints = torch.cat(positions, axis=0)
                ...
                # lower 8
                # full 55
                position = joints.reshape(n, 54 * 3)
                ...
                print("accel shape", acceleration.shape)

                # 15D
                pose_each_file = np.concatenate(
                    [pose_each_file, position, velocity, acceleration, angular_velocity], axis=1)

                print("pose_each_file shape", pose_each_file.shape)
                ...
                # # vel
                # dt = 1/30
                # joints = torch.from_numpy(positions)
                # velocities = (joints[1:] - joints[:-1]) / dt  # 前向差分
                # velocities = torch.cat([velocities[0:1], velocities], dim=0)  # 重复第一个速度以保持形状
                # velocities = velocities.numpy().reshape(n, 55*3)
                # print("vel shape", velocities.shape)
                #
                # # accelerations
                # dt = 1 / 30
                # joints = torch.from_numpy(positions).permute(1, 0, 2)
                # init_acc = (joints[:, 2:3] - 2 * joints[:, 1:2] + joints[:, :1]) / (dt ** 2)
                # middle_acc = (joints[:, 2:] - 2 * joints[:, 1:-1] + joints[:, :-2]) / (dt ** 2)
                # final_acc = (joints[:, -1:] - 2 * joints[:, -2:-1] + joints[:, -3:-2]) / (dt ** 2)
                # accelerations = torch.cat([init_acc, middle_acc, final_acc], dim=1).permute(1, 0, 2).cpu().numpy().reshape(n, 55*3)
                # print("acc shape", accelerations.shape)
                #
                # positions_flat = positions.reshape(n, 55*3)
                # # print("positions_flat",positions_flat.shape)
                #
                #
                # # 15D
                # pose_each_file = np.concatenate([pose_each_file, positions_flat, velocities, accelerations, angular_velocity], axis=1)
                # print("pos 15D shape", pose_each_file.shape)
                # 9D
                # pose_each_file = np.concatenate([pose_each_file, accelerations, angular_velocity], axis=1)
                # 9D_acc_rotvel
                # pose_each_file = np.concatenate([pose_each_file, accelerations, angular_velocity], axis=1)
                # 9D_position_vel
                # pose_each_file = np.concatenate([pose_each_file, velocities, positions_flat], axis=1)

                # print("rotation+contact+position+velocity+acceleration+rotation_vel",pose_each_file.shape)
            ...
            if self.args.audio_rep is not None:
                ...
                if not os.path.exists(audio_file):
                    ...
                    self.selected_file = self.selected_file.drop(self.selected_file[self.selected_file['id'] == id_pose].index)
                    continue
                audio_each_file, sr = librosa.load(audio_file,mono=True)
            ...
            if self.args.word_rep is not None:
                ...
                if self.args.t_pre_encoder == "bert":
                    ...
                    tokenizer = AutoTokenizer.from_pretrained(self.args.data_path_1+"bert-base-uncased/", local_files_only=True)
                    model = BertModel.from_pretrained(self.args.data_path_1+"bert-base-uncased/", local_files_only=True).eval()
                    list_word = []
                    all_hidden = []
                    max_len = 400
                    last = 0
    ...
    def _sample_from_clip(
        ...
        for ratio in self.args.multi_length_training:
            ...
            for i in range(num_subdivision): # cut into around 2s chip, (self npose)
                ...
                # print(f"pose:{sample_pose}")
                # print(f"word:{sample_word}")
                # print(f"sem:{sample_sem}")
                # print(f"sem shape:{sample_sem.shape}")
                # print(f"sem mean: {np.mean(sample_sem)}")

                # semantic data 0.188 0.2
                # if sample_pose.any() != None and np.mean(sample_sem)>0.188:
                ...
                try:
                    with dst_lmdb_env.begin(write=True) as txn:
                        for pose, audio, facial, shape, word, vid, emo, sem, trans in zip(
                            ...
                            if not success:
                                continue
                                # logger.error(f"false to write LMDB key: {k.decode('ascii')}")
                            else:
                                self.n_out_samples += 1
                                logger.debug(f"success to write LMDB key: {k.decode('ascii')}")
                except Exception as e:
                    logger.exception(f"LMDB exception:{e}")
                    raise e
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            sample = txn.get(key)
            if sample is None:
                logger.error(f"LMDB lack key:{key.decode('ascii')}")
                # raise ValueError(f"sample with key{key.decode('ascii')} is missing in LMDB")
            try:
                sample = pyarrow.deserialize(sample)
            except Exception as e:
                logger.exception(f"反序列化键 {key.decode('ascii')} 时发生异常: {e}")
            # add label
            ...
            if self.loader_type == "test":
                ...
                # # Calculate clip level label
                # label = 1 if sem.mean().item() > 0.39 else 0

                # # Filter to keep only data with label 1
                # if label == 1:
                ...
                labels = torch.zeros_like(sem, dtype=torch.long)
                labels[sem > 0.1] = 1
            return {"pose":tar_pose, "audio":in_audio, "facial":in_facial, "beta": in_shape, "word":in_word, "id":vid, "emo":emo, "sem":sem, "trans":trans, "label":labels}
        ...
        #         # Calculate clip level label
        #         # Weighted Average Semantic Score: 0.188
        #         label = 1 if sem.mean().item() > 0.188 else 0

        #         # Return only if label is 1
        #         if label == 1:
        #             return {
        #                 "pose": tar_pose,
        #                 "audio": in_audio,
        ...
        #             }
        #         else:
        #             # Increment idx and retry
        #             idx = (idx + 1) % len(self)  # Assuming len(self) gives dataset size
