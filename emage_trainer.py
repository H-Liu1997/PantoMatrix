...
class CustomTrainer(train.BaseTrainer):
    def __init__(self, args):
        ...
        self.tracker = other_tools.EpochTracker(
            ["fid", "l1div", "bc", "rec", "trans", "vel", "transv", 'dis', 'gen', 'acc', 'transa', 'exp', 'lvd', 'mse',
             "cls", "rec_face", "latent", "cls_full", "cls_self", "cls_word", "latent_word", "latent_self"],
        ...
        other_tools.load_checkpoints(self.vq_model_face, self.args.data_path_1 + "pretrained_vq/last_800_face_MR15D.bin",
                                     args.e_name)
        self.args.vae_test_dim = 78
        self.vq_model_upper = getattr(vq_model_module, "VQVAEConvZero")(self.args).to(self.rank)
        other_tools.load_checkpoints(self.vq_model_upper,
                                     self.args.data_path_1 + "pretrained_vq/last_500_upper_MR15D.bin", args.e_name)
        self.args.vae_test_dim = 180
        self.vq_model_hands = getattr(vq_model_module, "VQVAEConvZero")(self.args).to(self.rank)
        other_tools.load_checkpoints(self.vq_model_hands,
                                     self.args.data_path_1 + "pretrained_vq/last_700_hands_MR15D.bin", args.e_name)
        # 61
        # 15D 8j 55
        self.args.vae_test_dim = 55
        self.args.vae_layer = 4
        self.vq_model_lower = getattr(vq_model_module, "VQVAEConvZero")(self.args).to(self.rank)
        other_tools.load_checkpoints(self.vq_model_lower, self.args.data_path_1 + "pretrained_vq/last_800_lowerfoot_MR15D.bin",
                                     args.e_name)
        # 61
        # 15D 8j 55
        self.args.vae_test_dim = 55
        ...
        other_tools.load_checkpoints(self.global_motion, self.args.data_path_1 + "pretrained_vq/last_800_lower_MR15D.bin",
                                     args.e_name)
    ...
    def _load_data(self, dict_data):
        ...
        # j*9 --> j*8
        tar_pose_leg = rc.axis_angle_to_matrix(tar_pose_leg.reshape(bs, n, 8, 3))
        tar_pose_leg = rc.matrix_to_rotation_6d(tar_pose_leg).reshape(bs, n, 8 * 6)
        ...
        index_in = torch.stack([tar_index_value_upper_top, tar_index_value_hands_top, tar_index_value_lower_top],
                               dim=-1).long()
    ...
    def _g_training(self, loaded_data, use_adv, mode="train", epoch=0):
        ...
        #     index_loss_top_self = self.cls_loss(rec_index_face_self, tar_index_value_face_top) + self.cls_loss(
        #         rec_index_upper_self, tar_index_value_upper_top) + self.cls_loss(rec_index_lower_self,
        #                                                                          tar_index_value_lower_top) + self.cls_loss(
        #         rec_index_hands_self, tar_index_value_hands_top)
        ...
        #     index_loss_top_word = self.cls_loss(rec_index_face_word, tar_index_value_face_top) + self.cls_loss(
        #         rec_index_upper_word, tar_index_value_upper_top) + self.cls_loss(rec_index_lower_word,
        #                                                                          tar_index_value_lower_top) + self.cls_loss(
        #         rec_index_hands_word, tar_index_value_hands_top)
        #     self.tracker.update_meter("cls_word", "train", index_loss_top_word.item())
        #     g_loss_final += index_loss_top_word

        if mode != 'train':
            if self.args.cu != 0:
                _, rec_index_upper = torch.max(
                    rec_index_upper_val.reshape(-1, self.args.pose_length, self.args.vae_codebook_size), dim=2)
            ...
            if self.args.cl != 0:
                _, rec_index_lower = torch.max(
                    rec_index_lower_val.reshape(-1, self.args.pose_length, self.args.vae_codebook_size), dim=2)
            ...
            if self.args.ch != 0:
                _, rec_index_hands = torch.max(
                    rec_index_hands_val.reshape(-1, self.args.pose_length, self.args.vae_codebook_size), dim=2)
            ...
            if self.args.cf != 0:
                _, rec_index_face = torch.max(
                    rec_index_face_val.reshape(-1, self.args.pose_length, self.args.vae_codebook_size), dim=2)
            ...
            # j*9 --> j*8
            # print(f"rec_pose_legs shape{rec_pose_legs.shape}")
            rec_pose_lower = rec_pose_legs.reshape(bs, n, 8, 6)
            rec_pose_lower = rc.rotation_6d_to_matrix(rec_pose_lower)
            # j*9 --> j*8
            rec_pose_lower = rc.matrix_to_axis_angle(rec_pose_lower).reshape(bs * n, 8 * 3)
    ...
    def _g_test(self, loaded_data):
        ...
        for i in range(0, roundt):
            ...
            if self.args.cu != 0:
                ...
                _, rec_index_upper = torch.max(
                    rec_index_upper.reshape(-1, self.args.pose_length, self.args.vae_codebook_size), dim=2)
                # rec_upper = self.vq_model_upper.decode(rec_index_upper)
            else:
                _, rec_index_upper, _, _ = self.vq_model_upper.quantizer(net_out_val["rec_upper"])
                # rec_upper = self.vq_model_upper.decoder(rec_index_upper)
            if self.args.cl != 0:
                ...
                _, rec_index_lower = torch.max(
                    rec_index_lower.reshape(-1, self.args.pose_length, self.args.vae_codebook_size), dim=2)
                # rec_lower = self.vq_model_lower.decode(rec_index_lower)
            else:
                _, rec_index_lower, _, _ = self.vq_model_lower.quantizer(net_out_val["rec_lower"])
                # rec_lower = self.vq_model_lower.decoder(rec_index_lower)
            if self.args.ch != 0:
                ...
                _, rec_index_hands = torch.max(
                    rec_index_hands.reshape(-1, self.args.pose_length, self.args.vae_codebook_size), dim=2)
                # rec_hands = self.vq_model_hands.decode(rec_index_hands)
            else:
                _, rec_index_hands, _, _ = self.vq_model_hands.quantizer(net_out_val["rec_hands"])
                # rec_hands = self.vq_model_hands.decoder(rec_index_hands)
            if self.args.cf != 0:
                ...
                _, rec_index_face = torch.max(
                    rec_index_face.reshape(-1, self.args.pose_length, self.args.vae_codebook_size), dim=2)
                # rec_face = self.vq_model_face.decoder(rec_index_face)
            else:
                _, rec_index_face, _, _ = self.vq_model_face.quantizer(net_out_val["rec_face"])
                # rec_face = self.vq_model_face.decoder(rec_index_face)
            ...
            # j 8 --> j 9
            rec_pose_legs = rec_lower_last[:, :, 6:54]
        ...
        to_global[:, :, 54:57] = 0.0
        # j 8 --> j 9
        # to_global[:, :, 6:54] = rec_lower2global
