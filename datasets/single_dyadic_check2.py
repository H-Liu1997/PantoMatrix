...
class DyadicTalking(Dataset):
    ...
    def get_item(self, item):
        ...
        if audio_other_path is not None:
            ...
            audio_other_tensor = torch.ones(edx_audio - sdx_audio) * -1.0
        ...
        if np.random.rand() > self.cfg.data.random_mix:
            length = motion_dict["random_data"].shape[0] - self.cfg.model.pose_length
            ref_sdx = np.random.randint(0, length)
            ref_edx = ref_sdx + self.cfg.model.pose_length
        ...
        if motion_other_path is not None:
            ...
            if np.random.rand() > self.cfg.data.random_mix:
                length = motion_dict_other["random_data"].shape[0] - self.cfg.model.pose_length
                ref_sdx = np.random.randint(0, length)
                ref_edx = ref_sdx + self.cfg.model.pose_length
            ...
            motion_other_tensor, ref_motion_other_tensor = torch.ones(self.cfg.model.pose_length, self.cfg.data.motion_dim) * -1.0, torch.ones(self.cfg.model.pose_length, self.cfg.data.motion_dim) * -1.0
