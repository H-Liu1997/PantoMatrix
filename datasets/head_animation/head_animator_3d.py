...
class HeadAnimatorModule(BaseModule):
    ...
    def forward(self, source_img, target_img, masked_source_img, masked_target_img):
        if self.using_hybrid_mask:
            tgt_latent = self.motion_encoder(masked_target_img) # project target image to reference latent space
            ...
            print('tgt_latent', tgt_latent.shape)
            ...
            tgt_latent = self.motion_encoder(target_img) # project target image to reference latent space
    ...
    def compute_loss(self, img_target, img_target_recon, face_mask=None):
        ...
        loss = vgg_loss + l1_loss + face_loss
        loss_dict = {'loss': loss, 'l1_loss': l1_loss, 'vgg_loss': vgg_loss, 'face_loss': face_loss}
        return loss_dict
...
if __name__ == "__main__":
    ...
    from models.volumetric_avatar.img2vol_enc.py import LocalEncoder as FaceEncoder
